...
const data = [ /* 👉 paste your full JSON data here */
...
function SearchBox({ setLocation }) {
    ...
    const handleSearch = () => {
        ...
        // find by region, district, city or covered_area
    ...
    return (
        <div className="absolute top-4 left-4 z-[1000] bg-white shadow p-2 rounded">
            <input
                type="text"
                placeholder="Search region, district, city, area..."
...
function ChangeView({ coords }) {
    const map = useMap();
    if (coords) {
        map.setView(coords, 10); // zoom level 10
    }
    return null;
}

export default function BangladeshMap() {
    const [location, setLocation] = useState([23.8103, 90.4125]); // default Dhaka
    return (
        <div className="h-screen w-full flex items-center justify-center relative bg-gray-100">
          ...
          <MapContainer
            ...
            style={{ height: "70%", width: "70%" }} // bigger and centered
          >
            ...
            {data.map((place, idx) => (
              <Marker key={idx} position={[place.latitude, place.longitude]}>
                <Popup>
                  ...
                  {place.district}, {place.region} <br />
